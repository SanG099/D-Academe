...
const BuyToken = ({ tokensToBuy, setTokensToBuy, buyTokens, setLoading, saveTransaction, tokenPriceInEth }) => {
  const [error, setError] = useState(null);

  const handleBuyTokens = async () => {
    // Convert tokensToBuy to a number
    const tokensNumeric = parseFloat(tokensToBuy);

    if (isNaN(tokensNumeric) || tokensNumeric <= 0) {
    ...
    try {
      setLoading(true);
      const receipt = await buyTokens(tokensNumeric);
  ...
  return (
    <div className="flex flex-col items-center p-8 bg-gradient-to-r from-gray-700 to-gray-900 rounded-lg shadow-2xl mt-10 mx-auto max-w-md transform transition-all duration-300 hover:scale-105">
      ...
      >
        {setLoading ? 'Buying Tokens...' : 'Buy Tokens'}
      </button>
